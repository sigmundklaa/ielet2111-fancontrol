...
void PORT_init(void)
{
	...
	/* tacho (read) pins */
	PORTC.DIRCLR = PIN0_bm | PIN1_bm | PIN2_bm | PIN3_bm | PIN4_bm | PIN5_bm | PIN6_bm | PIN7_bm;
...
void TCB_init(void) {
	TCB0.CTRLA = TCB_CLKSEL_DIV1_gc | TCB_ENABLE_bm; // Set clock division factor to 1
	TCB0.CTRLB = TCB_CNTMODE_FRQ_gc | TCB_CCMPEN_bm; // Set TCB to frequency measurement mode and enable Compare/Capture output
	...
	EVSYS.CHANNEL2 = EVSYS_CHANNEL2_PORTC_PIN0_gc; // Change to the first tacho pin
}
...
void TCA0_init(void)
{
	...
	TCA0.SPLIT.CTRLB = TCA_SPLIT_HCMP0EN_bm | TCA_SPLIT_HCMP1EN_bm | TCA_SPLIT_HCMP2EN_bm/* enable compare channel
	  for the higher byte */
	 | TCA_SPLIT_LCMP0EN_bm | TCA_SPLIT_LCMP1EN_bm | TCA_SPLIT_LCMP2EN_bm; /* enable compare channel
	   for the lower byte */
...
void TCA1_init(void)
{
	...
	TCA1.SPLIT.CTRLB = TCA_SPLIT_HCMP0EN_bm | TCA_SPLIT_HCMP1EN_bm | TCA_SPLIT_HCMP2EN_bm/* enable compare channel
	  for the higher byte */
...
ISR(TCB0_INT_vect) {
	...
	pulse[current_tacho_pin] = TCB0.CCMP; // Store the new capture value for the current tacho pin

	rpm[current_tacho_pin] = ((1000000000UL)/(pulse[current_tacho_pin]*250*2))*60;
...
void fan_check_speed(uint8_t fan_index) {
	...
	*/
	...
	if (rpm[fan_index] < threshold - 1500) {
		printf("Error: Fan speed %d is too low, should be: %d\r\n", fan_index + 1, (int)threshold);
	}
}
